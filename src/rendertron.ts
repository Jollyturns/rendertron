...
export class Rendertron {
    ...
    async createRenderer(config: Config) {
        const browser = await puppeteer.launch({
            executablePath: config.googleChromePath || '/usr/bin/google-chrome',
    ...
    async initialize(config?: Config) {
        ...
        console.log("Will use configuration:\n", this.config);
        ...
        let logger = this.config.verboseLogging
            ? new KoaReqLogger().getMiddleware()
            : koaLogger();
        this.app.use(logger);
